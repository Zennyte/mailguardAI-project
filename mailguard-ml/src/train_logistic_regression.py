...
Trains a Logistic Regression classifier on the combined email dataset.
Tests multiple C values and picks the best one by macro F1-score.

Logistic Regression is the chosen final model for the platform because
it is fast, explainable, and returns calibrated probability scores.
"""
...
def save_confusion_matrix(y_test, y_pred, best_c):
    ...
    ax.set_title(f"Logistic Regression — Confusion Matrix (C={best_c})")
    plt.tight_layout()
...
def save_results_csv(results):
    ...
    path = os.path.join(RESULTS_DIR, "logistic_regression_results.csv")
...
def save_summary(results, best_c):
    ...
    path = os.path.join(RESULTS_DIR, "logistic_regression_summary.md")
